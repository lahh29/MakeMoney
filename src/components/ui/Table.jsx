...
export function Table({ columns = [], data = [], className = '' }) {
  return (
    <div style={{ overflowX: 'auto', width: '100%', WebkitOverflowScrolling: 'touch' }} role="region" aria-label="Tabla de datos" tabIndex={0}>
      <table className={className} style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '14px' }} role="table">
        <thead>
        ...
        <tbody>
          {data.length > 0 ? data.map((row, rowIndex) => (
            <tr key={rowIndex} style={{ borderBottom: '1px solid rgba(0,0,0,0.05)', transition: 'background-color 0.2s' }}>
